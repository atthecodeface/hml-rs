...
//a HmlResult, MarkupResult
//tp MarkupResult
/// The [MarkupResult] type is a result with an error type of
/// [crate::MarkupError]; it is abstract in the sense that it does not
/// refer to a specific point in a parser stream; rather it represents
/// errors such as bad namespace references in tag names
pub type MarkupResult<T> = std::result::Result<T, MarkupError>;

//tp HmlResult
/// The [HmlResult] type is a result with an error type of
/// [crate::HmlError] for a given position within a parsing stream
pub type HmlResult<T, P> = std::result::Result<T, HmlError<P>>;

//a MarkupError
//tp MarkupError
/// An error representing a tag or name issue when building HML
/// markup; this is not specifically related to a parsing position
/// (indeed, these errors can be generated by using the HML name
/// subsystem without any parsing).
#[derive(Debug, Error)]
...
impl MarkupError {
    //cp empty_name
    /// An 'empty' name was provided; names must always contain some
    /// characters; create the appropriate error
    pub fn empty_name() -> Self {
        Self::EmptyName {}
    }

    //cp unmapped_prefix
    /// Create a [MarkupError] for a specific namespace prefix that
    /// has not been mapped in the hierarchy of the document to this
    /// point
    ...
    P: Posn,
{
    ...
    pub fn span(&self) -> Option<&Span<P>> {
        match self {
            ...
            Self::ExpectedTagName { span, .. } => Some(span),
            Self::ExpectedWhitespaceAfterTag { span, .. } => Some(span),
        }
    }

    //cp map_markup_error
    /// Map a [MarkupResult] to an [HmlResult]; this passes an Ok
    /// value through unchanged, but it maps an Err to an
    /// HmlResult::MarkupError with the provided span
    #[inline(always)]
    ...
    P: Posn,
{
    ...
    // This creates a parse error for a particular location and
    // character, as required by the Lexer
