use crate::{HmlError, Name, StreamSpan, Reader, ReaderPosition};
...
impl <R:Reader> Error<R> {
    ...
    pub fn unexpected_attribute<T>(span:Span<R::Position>, name:Name) -> Result<R, T> {
        Err(Self::UnexpectedAttribute(span, name))
    }
    pub fn of_hml_error(span:Span<R::Position>, e:HmlError) -> Self {
        Self::HmlError(span, e)
    }
}
